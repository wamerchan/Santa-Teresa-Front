// Importaciones estáticas para evitar problemas con Vite
...
export const generateFinancialReport = async (
    ...
    try {
        ...
        return pdfMakeInstance.createPdf(docDefinition as any);
    } catch (error) {
...
export const generateServicesReport = async (services: ServiceData[]) => {
    try {
        // Verificar que PDFMake esté correctamente inicializado
        if (!initializePdfMake()) {
            throw new Error('PDFMake no está correctamente configurado');
        ...
        throw new Error('No se pudo generar el catálogo PDF. Verifica que PDFMake esté instalado correctamente.');
    }
};
